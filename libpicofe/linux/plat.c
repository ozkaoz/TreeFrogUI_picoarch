...
{
#if defined(PLATFORM_SF3000)
	int ret = plat_get_exe_dir(dst, len);
	snprintf(dst + ret, len - ret, ".picoarch/");
	mkdir(dst, 0755);
	
	char tmp[512];
	snprintf(tmp, sizeof(tmp), "%smds", dst);
	mkdir(tmp, 0755);
	snprintf(tmp, sizeof(tmp), "%ssram", dst);
	mkdir(tmp, 0755);
	snprintf(tmp, sizeof(tmp), "%sbios", dst);
	mkdir(tmp, 0755);
	
	return strlen(dst);
#elif !defined(__GP2X__) && !defined(PANDORA)
